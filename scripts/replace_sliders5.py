...
def process_file(filepath):
    ...
    pattern = re.compile(
        r'<div[^>]*className="[^"]*space-y-[1234][^"]*"[^>]*>\s*<div[^>]*className="flex justify-between[^"]*"[^>]*>\s*<span[^>]*>(.*?)</span>\s*<(?:span|strong)[^>]*>(.*?)</(?:span|strong)>\s*</div>\s*<input\s+type="range"\s+min="([^"]+)"\s+max="([^"]+)"\s+step="([^"]+)"\s+value=\{([^}]+)\}\s+onChange=\{\(.*?\)\s*=>\s*([a-zA-Z0-9_]+)\(.*?\}\s+className="[^"]+"\s*/>\s*(<p[^>]*>.*?</p>)?\s*</div>',
        re.DOTALL
    )
